 ...
 * Acts as the shim between internal use and the particular WALProvider we use to handle wal requests.
 *
 * Configure which provider gets used with the configuration setting "hbase.wal.provider". Available implementations:
 * <ul>
...
public class WALFactory {
  ...
  /**
   * Configuration-specific WAL writer
   */
  private final Class<? extends DefaultWALProvider.Writer> logWriterClass;
  ...
  private WALFactory(Configuration conf) {
    ...
    logWriterClass = conf.getClass("hbase.regionserver.hlog.writer.impl", ProtobufLogWriter.class,
        DefaultWALProvider.Writer.class);
    this.conf = conf;
  ...
  /**
   * Close all WALs from the MetaProvider.
   * In the current implementation, meta wals live in the same directory as non-meta so there is
   * no cleanup option; meta wals will be cleaned up when the non-meta ones are.
   */
  public void closeMeta() throws IOException {
  ...
  /**
   * Close all non-meta WALs and clean up any underlying storage.
   * In the current implementation, meta wals live in the same directory as non-meta so this
   * clean up will necessarily also impact non-meta ones. If you have used meta wals, you
   * must first call {@link #closeMeta}.
   */
  public void closeAndDelete() throws IOException {
    provider.closeAndDelete();
  }
   ...
   * to reopen it multiple times, use {@link WALProvider.Reader#reset()} instead of this method
   ...
   * @throws IOException
   */
  Writer createWALWriter(final FileSystem fs, final Path path) throws IOException {
    return createWriter(fs, path, false);
  }
  ...
  public Writer createRecoveredEditsWriter(final FileSystem fs, final Path path)
      throws IOException {
    return createWriter(fs, path, true);
  }

  private Writer createWriter(final FileSystem fs, final Path path,
      final boolean overwritable)
  throws IOException {
    try {
      DefaultWALProvider.Writer writer = logWriterClass.newInstance();
  ...
  static WALFactory getInstance(Configuration configuration) {
   ...
   * @return a WAL Reader, caller must close.
   */
  static Reader createReaderIgnoreCustomClass(final FileSystem fs, final Path path,
      final Configuration configuration) throws IOException {
   ...
   */
  static Writer createRecoveredEditsWriter(final FileSystem fs, final Path path, final Configuration configuration)
      throws IOException {
    return getInstance(configuration).createRecoveredEditsWriter(fs, path);
  }
  ...
  public static Writer createWALWriter(final FileSystem fs, final Path path, final Configuration configuration)
      throws IOException {
    return getInstance(configuration).createWALWriter(fs, path);
  }
}
